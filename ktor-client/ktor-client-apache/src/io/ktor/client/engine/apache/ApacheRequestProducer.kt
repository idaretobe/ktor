...
class ApacheRequestProducer(
    ...
    private fun prepareBody(): Job {
        val bodyChannel =
                if (body is OutgoingContent.ReadChannelContent) body.readFrom()
                else writer(Unconfined, autoFlush = true) {
                    val writeChannelBody = (body as OutgoingContent.WriteChannelContent)
                    writeChannelBody.writeTo(channel)
                }.channel
